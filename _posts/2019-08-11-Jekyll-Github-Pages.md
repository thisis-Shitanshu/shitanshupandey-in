...
- To host the project, add the project to the `gh-pages` branch.
    - *Sometimes their is a need to change the "baseurl" in _config.yml file. To load assets from the base of the repository.*
...
### Sitemap on Jekyll
- Sitemap is a XML which has link to all the posts and pages.
    - Sitemap helps bots look for all the links and idex them in the search engine DB.
    - The next setp is to submit the sitemap to all the webmasters tools.
...
### Disqus comments to Jekyll
- Sign up for Disqus:
    - Verify your email.
- Add Disqus code snippet to `_layouts/post.html`.

### MailChimp subscription in Jekyll
- Sign up for MailChimp:
    - Verify your email.
- Create List
- Generate Embeded Form
    - Copy the code.
- Create a `contact.md` file in the root.
    - Paste the embeded code with `layout` & `title` forntmatter.
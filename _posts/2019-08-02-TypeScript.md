...
# Type system
- Type systems increase code quality, readability and make it easier to maintain and refactor codebase.
- Errors can be caught at compile time rather than at run time.
    - *A compiler error typically tells you exactly where something went wrong and what exactly went wrong whereas a runtime error is accompanied by a stack trace that may be misleading and results on significant amount of time spent on debug work.*
...
## Tuple
Tuple types allow you to express an organised array where the type of a fixed number of elements is known.
```typescript
...
## Null and Undefined
They're not extremely useful on theor own but they become useful when used within union types.
```typescript
...
No operations are permitted on an **unknown** withut first asserting or narrowing to a more specific type.
```typescript
...
## Type Alias
They provides names for type annotations allowing you to use it in several places.
```typescript
    type Login = string;
```
...
- Optional properties are decalared with **?**. Meaning, that object of the interface may or may not define these properties.
...
# Enums
An **enum** is a way to organise a collection of related values that can be numeric or string values.
```typescript
    enum CardSuit {
        Clubs,
        Diamonds,
...
- **Abtract classes** cannot be directly instantiated. They are mainly for inheritance where the class which extends the abstract class must define all the abstract methods.
- **Abstract members** do not contain an implementation, thus cannot be directly accessed. These memebers must be implmented in child classes.


# Type Assertion
Typescript allows to override its inferred types in any way.
This is used when having a better undrstanding of a variable type than the compiler on its own.
- Type assertion are usually used when migrating code from JavaScript.
...
# Type Compatibility
Type compatibility is based on structural typing, which relates types based soley on their members.
...
## in
The **in** operator checks for existence of aproperty on an object.
...
## Conditional Types
A conditional type describes a type realtionship test and selects one of two possible types, depending on the outcome of that test.
```typescript
...
# Mapped Types
It allows to create new types from existing ones by mapping over property types.
Each property of the existing type is transofrmed according to a rule that you specify.
